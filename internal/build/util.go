...
// any error.
func MustRun(cmd *exec.Cmd, isPanic bool) {
	...
	fmt.Println("<<< (done)", strings.Join(cmd.Args, " "))
}
...
// vendored packages.
func ExpandPackagesNoIgnoreDirs(patterns []string) []string {
	expand := false
	...
	if expand {
		...
		if err != nil {
			log.Printf("package listing failed: %v\n%s\n", err, string(out))
		}
		var packages []string
		for _, line := range strings.Split(string(out), "\n") {
			if !strings.Contains(line, "/vendor/") && !strings.Contains(line, "cc/") && !strings.Contains(line, "/go-ethereum") && !strings.Contains(line, "/pttai.js") && !strings.Contains(line, "/e2e") {
				log.Printf("to add package: line: %s\n", line)
