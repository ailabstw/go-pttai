...
func TestFriendPttoplog(t *testing.T) {
	NNodes = 2
	isDebug := true
	...
	dataPttOplogList0_13 := &struct {
		Result []*pkgservice.PttOplog `json:"result"`
	}{}
	testListCore(t0, bodyString, dataPttOplogList0_13, t, isDebug)
	assert.Equal(1, len(dataPttOplogList0_13.Result))

	dataPttOplogList1_13 := &struct {
		Result []*pkgservice.PttOplog `json:"result"`
	}{}
	testListCore(t1, bodyString, dataPttOplogList1_13, t, isDebug)
	assert.Equal(1, len(dataPttOplogList1_13.Result))
}
