...
func TestFriendDeleteArticle(t *testing.T) {
	NNodes = 2
	isDebug := true
	...
	assert.Equal(types.StatusDeleted, article0_49_0.Status)
	...
	assert.Equal(types.StatusDeleted, article1_49_0.Status)
	...
	assert.Equal(0, len(dataGetArticleBlockList0_50.Result))

	dataGetArticleBlockList1_50 := &struct {
		Result []*content.ArticleBlock `json:"result"`
	}{}
	testListCore(t1, bodyString, dataGetArticleBlockList1_50, t, isDebug)
	assert.Equal(0, len(dataGetArticleBlockList1_50.Result))
}
