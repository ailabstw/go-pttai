...
func TestLeaderOnlyCommitsLogFromCurrentTerm(t *testing.T) {
	...
	log.Debug("TestLeaderOnlyCommitsLogFromCurrentTerm: start")
	defer log.Debug("TestLeaderOnlyCommitsLogFromCurrentTerm: done")
	...
	for i, tt := range tests {
		...
		log.Debug("TestLeaderOnlyCommitsLogFromCurrentTerm: after becomeLeader", "r", r, "committed", r.raftLog.committed)
		for i, p := range r.prs {
			log.Debug("TestLeaderOnlyCommitsLogFromCurrentTerm: after becomeLeader", "r", r.id, "i", i, "p", p)
		}

		r.readMessages()

		log.Debug("TestLeaderOnlyCommitsLogFromCurrentTerm: after readMessages", "r", r, "committed", r.raftLog.committed)
		for i, p := range r.prs {
			log.Debug("TestLeaderOnlyCommitsLogFromCurrentTerm: after readMessages", "r", r.id, "i", i, "p", p)
		}

		// propose a entry to current term
		r.Step(pb.Message{From: 1, To: 1, Type: pb.MsgProp, Entries: []pb.Entry{{}}})

		log.Debug("TestLeaderOnlyCommitsLogFromCurrentTerm: after Step Prop", "r", r, "committed", r.raftLog.committed)

		for i, p := range r.prs {
			log.Debug("TestLeaderOnlyCommitsLogFromCurrentTerm: after Step Prop", "r", r.id, "i", i, "p", p)
		}

		r.Step(pb.Message{From: 2, To: 1, Type: pb.MsgAppResp, Term: r.Term, Index: tt.index})

		log.Debug("TestLeaderOnlyCommitsLogFromCurrentTerm: after Step MsgAppResp", "r", r)
