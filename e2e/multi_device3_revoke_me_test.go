...
func TestMultiDevice3RevokeMe(t *testing.T) {
	NNodes = 3
	isDebug := true
	...
	isOk := false
	_, myErr = testCore(t0, bodyString, &isOk, t, isDebug)
	assert.Equal(true, isOk)
	assert.Equal(0, myErr.Code)
	assert.Equal("", myErr.Msg)
	...
	me2_10_4 := &me.MyInfo{}
	testCore(t2, bodyString, me2_10_4, t, isDebug)
	assert.Equal(types.StatusDeleted, me2_10_4.Status)
	assert.Equal(me1_3.ID, me2_10_4.ID)
	assert.Equal(1, len(me2_10_4.OwnerIDs))
	assert.Equal(me1_3.ID, me2_10_4.OwnerIDs[0])
	assert.Equal(true, me2_10_4.IsOwner(me1_3.ID))
}
