...
func TestFriendCard(t *testing.T) {
	NNodes = 2
	isDebug := true
	...
	bodyString = fmt.Sprintf(`{"id": "testID", "method": "me_setMyNameCard", "params": ["%v"]}`, marshaledStr)

	dataSetMyName0_5_1 := &account.NameCard{}
	...
	dataSetMyName1_5_1 := &account.NameCard{}
	...
	dataGetNameCard0_5_2 := &account.NameCard{}
	testCore(t0, bodyString, dataGetNameCard0_5_2, t, isDebug)
	assert.Equal(me0_1.ID, dataGetNameCard0_5_2.ID)
	assert.Equal(myName, dataGetNameCard0_5_2.Card)

	t.Logf("5.2 get user name: t1")
	marshaled, _ = me1_1.ID.MarshalText()
	bodyString = fmt.Sprintf(`{"id": "testID", "method": "account_getRawNameCard", "params": ["%v"]}`, string(marshaled))

	dataGetNameCard1_5_2 := &account.NameCard{}
	testCore(t1, bodyString, dataGetNameCard1_5_2, t, isDebug)
	assert.Equal(me1_1.ID, dataGetNameCard1_5_2.ID)
	assert.Equal(myName1, dataGetNameCard1_5_2.Card)
	...
	// 13.
	marshaled, _ = me1_1.ID.MarshalText()
	bodyString = fmt.Sprintf(`{"id": "testID", "method": "account_getNameCard", "params": ["%v"]}`, string(marshaled))

	friend0_13 := &account.BackendNameCard{}
	testCore(t0, bodyString, friend0_13, t, isDebug)

	assert.Equal(me1_1.ID, friend0_13.ID)
	assert.Equal(myName1, friend0_13.Card)

	marshaled, _ = me0_1.ID.MarshalText()
	bodyString = fmt.Sprintf(`{"id": "testID", "method": "account_getNameCard", "params": ["%v"]}`, string(marshaled))

	friend1_13 := &account.BackendNameCard{}
	testCore(t1, bodyString, friend1_13, t, isDebug)

	assert.Equal(me0_1.ID, friend1_13.ID)
	assert.Equal(myName, friend1_13.Card)
}
