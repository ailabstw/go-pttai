...
func TestContentUpdateArticle(t *testing.T) {
	NNodes = 1
	isDebug := true
	...
	// 39. update-article
	article39, _ := json.Marshal([]string{
	...
	bodyString = fmt.Sprintf(`{"id": "testID", "method": "content_updateArticle", "params": ["%v", "%v", %v, []]}`, string(marshaledID), string(marshaledID2), string(article39))
	dataUpdateArticle0_39 := &content.BackendUpdateArticle{}
	testCore(t0, bodyString, dataUpdateArticle0_39, t, isDebug)
	assert.Equal(board0_10_0.ID, dataUpdateArticle0_39.BoardID)
	assert.Equal(article0_36.ID, dataUpdateArticle0_39.ArticleID)
	assert.Equal(2, dataUpdateArticle0_39.NBlock)
	...
	marshaledID3, _ = article0_40.ContentBlockID.MarshalText()
