...
func testLeaderElectionOverwriteNewerLogs(t *testing.T, preVote bool) {
	...
	log.Debug("testLeaderElectionOverwriteNewerLogs: start", "preVote", preVote)
	defer log.Debug("testLeaderElectionOverwriteNewerLogs: done", "preVote", preVote)
	...
	log.Debug("testLeaderElectionOverwriteNewerLogs", "r1", r1, "committed", r1.raftLog.committed, "applied", r1.raftLog.applied)
...
func TestLogReplication(t *testing.T) {
	...
	log.Debug("TestLogReplication: start")
	...
	for i, tt := range tests {
		log.Debug("TestLogReplication: (in loop)", "i", i)
		tt.send(pb.Message{From: 1, To: 1, Type: pb.MsgHup})
	...
	log.Debug("TestLogReplication: done")
}
...
func TestSingleNodeCommit(t *testing.T) {
	...
	log.Debug("TestSingleNodeCommit: start")
	...
	log.Debug("TestSingleNodeCommit: done")
}
...
func TestDuelingCandidates(t *testing.T) {
	...
	log.Debug("TestDuelingCandidates: start")
	...
	log.Debug("TestDuelingCandidates: done")
}
...
func TestDuelingPreCandidates(t *testing.T) {
	...
	log.Debug("TestDuelingPreCandidates: start")
	...
	log.Debug("TestDuelingPreCandidates: done")
}
...
func TestCandidateConcede(t *testing.T) {
	...
	log.Debug("TestCandidateConcede: start")
	...
	log.Debug("TestCandidateConcede: done")
}
...
func TestSingleNodeCandidate(t *testing.T) {
	...
	log.Debug("TestSingleNodeCandidate: start")
	...
	log.Debug("TestSingleNodeCandidate: done")
}
...
func TestSingleNodePreCandidate(t *testing.T) {
	...
	log.Debug("TestSingleNodePreCandidate: start")
	...
	log.Debug("TestSingleNodePreCandidate: done")
}
...
func TestProposalByProxy(t *testing.T) {
	...
	log.Debug("TestProposalByProxy: start")
	...
	log.Debug("TestProposalByProxy: done")
}
...
func TestCommit(t *testing.T) {
	...
	for i, tt := range tests {
		...
		for j := 0; j < len(tt.matches); j++ {
			sm.setProgress(uint64(j)+1, tt.matches[j], tt.matches[j]+1, false, 1, true)
		}
		//sm.calcTotalWeight()
		sm.maybeCommit()
...
func TestPastElectionTimeout(t *testing.T) {
	...
	log.Debug("TestPastElectionTimeout: start")
	...
	log.Debug("TestPastElectionTimeout: done")
}
...
func TestRaftFreesReadOnlyMem(t *testing.T) {
	...
	log.Debug("TestRaftFreesReadOnlyMem: start")
	defer log.Debug("TestRaftFreesReadOnlyMem: done")
...
func TestRestore(t *testing.T) {
	s := pb.Snapshot{
		Metadata: pb.SnapshotMetadata{
			...
			ConfState: pb.ConfState{Nodes: []uint64{1, 2, 3}, Weights: []uint32{1, 1, 1}},
		},
	}
	...
	sg, weights := sm.nodes(false)
	if !reflect.DeepEqual(sg, s.Metadata.ConfState.Nodes) {
		t.Errorf("sm.Nodes = %+v, want %+v", sg, s.Metadata.ConfState.Nodes)
	}

	if !reflect.DeepEqual(weights, s.Metadata.ConfState.Weights) {
		t.Errorf("sm.Weights = %+v, want %+v", weights, s.Metadata.ConfState.Weights)
	}
...
func TestRestoreWithLearner(t *testing.T) {
	s := pb.Snapshot{
		Metadata: pb.SnapshotMetadata{
			Index:     11, // magic number
			Term:      11, // magic number
			ConfState: pb.ConfState{Nodes: []uint64{1, 2}, Learners: []uint64{3}, Weights: []uint32{1, 1}},
		},
	}
	...
	if len(weights) != len(s.Metadata.ConfState.Weights) {
		t.Errorf("sm.Weights = %+v, length not equal with %+v", weights, s.Metadata.ConfState.Weights)
	}
...
func TestAddNode(t *testing.T) {
	...
	myAssert.Equal([]uint32{1}, weights)
	...
	if !reflect.DeepEqual(weights, wweights) {
		t.Errorf("weights = %v, want %v", weights, wweights)
	}
}
...
func TestAddNodeCheckQuorum(t *testing.T) {
	...
	log.Debug("TestAddNodeCheckQuorum: start")
	defer log.Debug("TestAddNodeCheckQuorum: done")
...
func TestRaftNodes(t *testing.T) {
	...
	for i, tt := range tests {
		...
		nodes, weights := r.nodes(false)
		if !reflect.DeepEqual(nodes, tt.wids) {
			t.Errorf("#%d: nodes = %+v, want %+v", i, nodes, tt.wids)
		}

		if !reflect.DeepEqual(weights, tt.wweights) {
			t.Errorf("#%d: weights = %+v, want %+v", i, weights, tt.wweights)
		}
	}
}
			...
			if len(v.prs) == 0 {
				...
				for i := 0; i < size; i++ {
					if _, ok := learners[peerAddrs[i]]; ok {
						...
						v.prs[peerAddrs[i]] = &Progress{Weight: 1}
					}
				}
			}
			v.reset(v.Term)
			log.Debug("newNetworkWithConfig", "v", v)
			npeers[id] = v
		case *blackHole:
			npeers[id] = v
		default:
...
func newTestConfig(id uint64, peerList []uint64, election, heartbeat int, storage Storage) *Config {
...
func newTestRaft(id uint64, peers []uint64, election, heartbeat int, storage Storage) *raft {
	...
	r := newRaft(cfg)
	log.Debug("newTestRaft", "cfg", cfg, "r", r)
	for i, pr := range r.prs {
		log.Debug("newTestRaft: prs", "i", i, "pr", pr)
	}
	return r
}
