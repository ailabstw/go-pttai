...
func newRaft(c *Config) *raft {
	...
	for p, pr := range r.prs {
		log.Debug("newRaft: prs", "r", r.id, "p", p, "pr", pr)
	}
	...
	for p, pr := range r.prs {
		log.Debug("newRaft: to becomeFollower: prs", "p", p, "pr", pr)
	}
	r.becomeFollower(r.Term, None)

	for p, pr := range r.prs {
		log.Debug("newRaft: after becomeFollower: prs", "p", p, "pr", pr)
	}
...
func (r *raft) nodes(isLocked bool) ([]uint64, []uint32) {
	...
	weights := make([]uint32, len(r.prs))
	for id, _ := range r.prs {
		nodes = append(nodes, id)
	}
	sort.Sort(uint64Slice(nodes))
	for i, id := range nodes {
		weights[i] = r.prs[id].Weight
	}
...
func (r *raft) getProgress(id uint64, isLocked bool) *Progress {
	...
	log.Debug("getProgress: start", "id", id)
...
func (r *raft) maybeCommit() bool {
	...
	// we would like to know the match of the quorum.
	...
	for i, p := range mis {
		log.Debug("maybeCommit", "r", r.id, "i", i, "p", p)
	}
	...
	log.Debug("maybeCommit", "r", r.id, "mci", mci, "term", r.Term)

	// sorted matched from lower to large. we would like to know the match of the quorum
	//
...
func (r *raft) mustCommit() bool {
	...
	log.Debug("mustCommit: to raftLog.maybeCommit", "mci", mci, "term", r.Term)
...
func (r *raft) reset(term uint64) {
	...
	for _, pr := range r.prs {
		log.Debug("reset: before forEachProgress", "r", r.id, "pr", pr)
	}
	...
	for _, pr := range r.prs {
		log.Debug("reset: after forEachProgress", "r", r.id, "pr", pr)
	}
...
func (r *raft) becomeFollower(term uint64, lead uint64) {
	...
	for _, pr := range r.prs {
		log.Debug("becomeFollower: before reset", "r", r.id, "pr", pr)
	}

	r.reset(term)

	for _, pr := range r.prs {
		log.Debug("becomeFollower: after reset", "r", r.id, "pr", pr)
	}
...
func (r *raft) becomeCandidate() {
	...
	for _, pr := range r.prs {
		log.Debug("becomeCandidate: before reset", "r", r.id, "pr", pr)
	}

	r.reset(r.Term + 1)

	for _, pr := range r.prs {
		log.Debug("becomeCandidate: after reset", "r", r.id, "pr", pr)
	}
...
func (r *raft) becomeLeader() {
	...
	for _, pr := range r.prs {
		log.Debug("becomeLeader: before reset", "r", r.id, "pr", pr)
	}

	r.reset(r.Term)

	for _, pr := range r.prs {
		log.Debug("becomeLeader: after reset", "r", r.id, "pr", pr)
	}
...
func (r *raft) campaign(t CampaignType) {
	for _, pr := range r.prs {
		log.Debug("campaign: start", "r", r.id, "pr", pr)
	}
	...
	granted, _ := r.poll(r.id, voteRespMsgType(voteMsg), true)
	if granted >= r.quorum() {
...
func (r *raft) poll(id uint64, t pb.MessageType, v bool) (int, int) {
	if v {
		r.logger.Infof("%x received %s from %x at term %d", r.id, t, id, r.Term)
		log.Debug("poll: cast", "r", r.id, "t", t, "id", id, "term", r.Term, "totalWeight", r.totalWeight)
	} else {
		r.logger.Infof("%x received %s rejection from %x at term %d", r.id, t, id, r.Term)
		log.Debug("poll: reject", "r", r.id, "t", t, "id", id, "term", r.Term, "totalWeight", r.totalWeight)
	}
	...
	for eachID, vv := range r.votes {
		...
		log.Debug("poll (in-for-loop)", "eachID", eachID, "pr", pr)
...
func (r *raft) Step(m pb.Message) error {
	/*
		log.Debug("Step: start", "r", r.id, "m", m)

		for _, pr := range r.prs {
			log.Debug("Step: start", "r", r.id, "pr", pr)
		}
	*/
	...
	case pb.MsgHup:
		if r.state != StateLeader {
			for _, pr := range r.prs {
				log.Debug("Step: in MsgHup", "r", r.id, "pr", pr)
			}
...
func stepLeader(r *raft, m pb.Message) error {
	...
	case pb.MsgHeartbeatResp:
		...
		r.logger.Infof("Recved pb.MsgHeartbeatResp: pr: %v", pr)
		...
		ackCount := r.readOnly.recvAck(m, r.prs)
		log.Debug("stepLeader: MsgHeartbeatResp", "ackCount", ackCount, "quorum", r.quorum())
...
func stepCandidate(r *raft, m pb.Message) error {
	...
	case pb.MsgSnap:
		...
		r.handleSnapshot(m)
	case myVoteRespType:
		gr, rej := r.poll(m.From, m.Type, !m.Reject)
		quorum := r.quorum()
...
func (r *raft) handleForceProp(m pb.Message) error {
	log.Debug("handleForceProp: start")
	if r.state != StateLeader {
		if r.state == StateFollower {
			r.becomeCandidate()
		}
...
func (r *raft) restoreNode(nodes []uint64, weights []uint32, isLearner bool, isLocked bool) {
	...
	log.Debug("restoreNode: after pass lock")

	var weight uint32
	for i, n := range nodes {
		// XXX hack for nil weights
		weight = 1
		if weights != nil {
			weight = weights[i]
		}
		...
		r.setProgress(n, match, next, isLearner, weight, false)
		r.logger.Infof("%x restored progress of %x [%s]", r.id, n, r.getProgress(n, true))
	}
	r.calcTotalWeight()
}

func (r *raft) calcTotalWeight() {
	r.totalWeight = 0
...
func (r *raft) addNodeOrLearnerNode(id uint64, isLearner bool, weight uint32, isLocked bool) {
	...
	if pr == nil {
		r.setProgress(id, 0, r.raftLog.lastIndex()+1, isLearner, weight, false)
	} else {
		...
		pr.Weight = weight
		r.prs[id] = pr
		log.Debug("addNodeOrLearnerNode", "r", r.id, "id", id, "pr", pr)
	}
	r.calcTotalWeight()
...
func (r *raft) removeNode(id uint64) {
	...
	// be committed.
	log.Info("removeNode: to check commit", "id", id)
	if r.maybeCommit() {
		log.Info("removeNode: committed", "id", id)
		r.bcastAppend()
	}
...
func (r *raft) setProgress(id, match, next uint64, isLearner bool, weight uint32, isCalc bool) {
	log.Debug("setProgress: start", "r", r.id, "id", id, "weight", weight, "isCalc", isCalc)
	defer func(isCalc bool) {
		if isCalc {
			log.Debug("setProgress: to calcTotalWeight")
			r.calcTotalWeight()
		}
	}(isCalc)
