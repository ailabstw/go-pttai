...
func TestFriendUpdateArticle2(t *testing.T) {
	NNodes = 2
	isDebug := true
	...
	dataGetArticleBlockList0_45_1 := &struct {
		Result []*content.ArticleBlock `json:"result"`
	}{}
	testListCore(t0, bodyString, dataGetArticleBlockList0_45_1, t, isDebug)
	assert.Equal(4, len(dataGetArticleBlockList0_45_1.Result))

	dataGetArticleBlockList1_45_1 := &struct {
		Result []*content.ArticleBlock `json:"result"`
	}{}
	...
	// 48.1. test-error
	err0_48_1 := testError("http://127.0.0.1:9450")
	assert.NotEqual(nil, err0_48_1)
	...
	assert.Equal(types.StatusPending, syncInfo1_50.GetStatus())
