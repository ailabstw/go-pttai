...
func TestFriendListSeen(t *testing.T) {
	NNodes = 2
	isDebug := true
	...
	// 13. mark friend list seen
	bodyString = fmt.Sprintf(`{"id": "testID", "method": "friend_markFriendListSeen"}`)

	var ts0_13 types.Timestamp
	testCore(t0, bodyString, &ts0_13, t, isDebug)

	// 14. get friend list seen
	bodyString = fmt.Sprintf(`{"id": "testID", "method": "friend_getFriendListSeen"}`)

	var ts0_14 types.Timestamp
	testCore(t0, bodyString, &ts0_14, t, isDebug)
	assert.Equal(ts0_13, ts0_14)
}
