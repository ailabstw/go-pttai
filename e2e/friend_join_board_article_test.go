...
func TestFriendJoinBoardArticle(t *testing.T) {
	NNodes = 2
	isDebug := true
	...
	assert.Equal(dataCreateBoard0_13.ID, dataCreateArticle0_13_1.BoardID)
	...
	// 18. content-get-article-list
	marshaledID, _ = dataCreateBoard0_13.ID.MarshalText()

	bodyString = fmt.Sprintf(`{"id": "testID", "method": "content_getArticleList", "params": ["%v", "", 0, 2]}`, string(marshaledID))
	dataGetArticleList0_18 := &struct {
		Result []*content.BackendGetArticle `json:"result"`
	}{}
	testListCore(t0, bodyString, dataGetArticleList0_18, t, isDebug)
	assert.Equal(1, len(dataGetArticleList0_18.Result))

	dataGetArticleList1_18 := &struct {
		Result []*content.BackendGetArticle `json:"result"`
	}{}
	testListCore(t1, bodyString, dataGetArticleList1_18, t, isDebug)
	assert.Equal(1, len(dataGetArticleList1_18.Result))

	// 19. get-article-block
	marshaledID, _ = dataCreateBoard0_13.ID.MarshalText()
	marshaledID2, _ = article0_13_2.ID.MarshalText()
	marshaledID3, _ = article0_13_2.ContentBlockID.MarshalText()
	...
	dataGetArticleBlockList0_19 := &struct {
		Result []*content.ArticleBlock `json:"result"`
	}{}
	testListCore(t0, bodyString, dataGetArticleBlockList0_19, t, isDebug)
	assert.Equal(3, len(dataGetArticleBlockList0_19.Result))

	assert.Equal(article0, dataGetArticleBlockList0_19.Result[0].Buf)
	assert.Equal(article1, dataGetArticleBlockList0_19.Result[1].Buf)
	assert.Equal(article2, dataGetArticleBlockList0_19.Result[2].Buf)

	dataGetArticleBlockList1_19 := &struct {
		Result []*content.ArticleBlock `json:"result"`
	}{}
	testListCore(t1, bodyString, dataGetArticleBlockList1_19, t, isDebug)
	assert.Equal(3, len(dataGetArticleBlockList1_19.Result))

	assert.Equal(article0, dataGetArticleBlockList1_19.Result[0].Buf)
	assert.Equal(article1, dataGetArticleBlockList1_19.Result[1].Buf)
	assert.Equal(article2, dataGetArticleBlockList1_19.Result[2].Buf)
}
