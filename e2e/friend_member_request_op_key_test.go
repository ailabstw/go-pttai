...
func TestFriendMemberRequestOpKey(t *testing.T) {
	NNodes = 2
	isDebug := true
	...
	// 18. get board op-key
	marshaled, _ = dataCreateBoard0_13.ID.MarshalText()
	bodyString = fmt.Sprintf(`{"id": "testID", "method": "content_getOpKeyInfos", "params": ["%v"]}`, string(marshaled))

	dataGetKeyInfos0_18 := &struct {
		Result []*pkgservice.KeyInfo `json:"result"`
	}{}
	testListCore(t0, bodyString, dataGetKeyInfos0_18, t, isDebug)
	assert.Equal(1, len(dataGetKeyInfos0_18.Result))

	dataGetKeyInfos1_18 := &struct {
		Result []*pkgservice.KeyInfo `json:"result"`
	}{}
	testListCore(t1, bodyString, dataGetKeyInfos1_18, t, isDebug)
	assert.Equal(1, len(dataGetKeyInfos1_18.Result))
	...
	err0_19 := testError("http://127.0.0.1:9450")
	assert.NotEqual(nil, err0_19)

	err1_19 := testError("http://127.0.0.1:9451")
	assert.NotEqual(nil, err1_19)
	...
	dataGetKeyInfos1_21 := &struct {
		Result []*pkgservice.KeyInfo `json:"result"`
	}{}
	testListCore(t1, bodyString, dataGetKeyInfos1_21, t, isDebug)
	...
	dataGetKeyInfos0_23 := &struct {
		Result []*pkgservice.KeyInfo `json:"result"`
	}{}
	testListCore(t0, bodyString, dataGetKeyInfos0_23, t, isDebug)
	assert.Equal(1, len(dataGetKeyInfos0_23.Result))
	...
	bool1_24 := false
	testCore(t1, bodyString, &bool1_24, t, isDebug)
	...
	dataGetKeyInfos0_25 := &struct {
		Result []*pkgservice.KeyInfo `json:"result"`
	}{}
	testListCore(t0, bodyString, dataGetKeyInfos0_25, t, isDebug)
	assert.Equal(1, len(dataGetKeyInfos0_25.Result))
}
