...
func TestFriendPersonalBoardContent(t *testing.T) {
	NNodes = 2
	isDebug := true
	...
	assert.Equal(2, len(dataBoardList0_16.Result))
	...
	assert.Equal(2, len(dataBoardList1_16.Result))
	...
	assert.Equal(pkgservice.EntityTypePersonal, dataBoard1_16_0.BoardType)

	assert.Equal(types.StatusAlive, dataBoard1_16_1.Status)
	assert.Equal(pkgservice.EntityTypePersonal, dataBoard1_16_1.BoardType)

	// assert.Equal(types.StatusAlive, dataBoard1_16_2.Status)
	// assert.Equal(pkgservice.EntityTypePrivate, dataBoard1_16_2.BoardType)
	...
	marshaledID, _ = dataBoard1_16_1.ID.MarshalText()
	...
	assert.Equal(dataBoard1_16_1.ID, dataCreateArticle0_35.BoardID)
