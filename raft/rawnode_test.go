...
func TestRawNodeStep(t *testing.T) {
	for i, msgn := range raftpb.MessageType_name {
		s := NewMemoryStorage()
		rawNode, err := NewRawNode(newTestConfig(1, nil, 10, 1, s), []Peer{{ID: 1, Weight: 1}})
		if err != nil {
			t.Fatal(err)
		}
...
func TestRawNodeProposeAddDuplicateNode(t *testing.T) {
	...
	log.Debug("TestRawNodeProposeAddDuplicateNode: start")
	defer log.Debug("TestRawNodeProposeAddDuplicateNode: end")
	...
	if !bytes.Equal(entries[1].Data, ccdata1) {
		t.Errorf("entries[1].Data = %v, want %v", entries[1].Data, ccdata1)
	}
...
func TestRawNodeReadIndex(t *testing.T) {
	...
	rawNode, err := NewRawNode(c, []Peer{{ID: 1, Weight: 1}})
	if err != nil {
		t.Fatal(err)
	}
...
func TestRawNodeStart(t *testing.T) {
	...
	rawNode, err := NewRawNode(newTestConfig(1, nil, 10, 1, storage), []Peer{{ID: 1, Weight: 1}})
	if err != nil {
		t.Fatal(err)
	}
	rd := rawNode.Ready()
