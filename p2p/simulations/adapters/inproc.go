...
func (s *SimAdapter) NewNode(config *NodeConfig) (Node, error) {
	...
	simNode := &SimNode{
		...
		node:    n,
		adapter: s,
		running: make(map[string]pkgservice.PttService),
	}
	s.nodes[id] = simNode
	return simNode, nil
}
...
type SimNode struct {
	...
	adapter      *SimAdapter
	node         *node.Node
	running      map[string]pkgservice.PttService
	client       *rpc.Client
...
func (sn *SimNode) Snapshots() (map[string][]byte, error) {
	sn.lock.RLock()
	services := make(map[string]pkgservice.PttService, len(sn.running))
...
func (sn *SimNode) Start(snapshots map[string][]byte) error {
	newService := func(name string) func(ctx *pkgservice.ServiceContext) (pkgservice.PttService, error) {
		return func(nodeCtx *pkgservice.ServiceContext) (pkgservice.PttService, error) {
			ctx := &ServiceContext{
...
func (sn *SimNode) Stop() error {
	...
	sn.lock.Unlock()
	return sn.node.Stop(false, false)
}

// Services returns a copy of the underlying services
func (sn *SimNode) Services() []pkgservice.PttService {
	sn.lock.RLock()
	defer sn.lock.RUnlock()
	services := make([]pkgservice.PttService, 0, len(sn.running))
