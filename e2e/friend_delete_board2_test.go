...
func TestFriendDeleteBoard2(t *testing.T) {
	NNodes = 2
	isDebug := true
	...
	assert.Equal(board1_16_2.ID, dataCreateArticle1_44.BoardID)
	...
	err1_2 := testError("http://127.0.0.1:9451")
	assert.NotEqual(nil, err1_2)

	// 47. content-delete-board
	marshaledID, _ = board1_16_2.ID.MarshalText()

	bodyString = fmt.Sprintf(`{"id": "testID", "method": "content_deleteBoard", "params": ["%v"]}`, string(marshaledID))

	bool0_47 := false
	testCore(t0, bodyString, &bool0_47, t, isDebug)

	assert.Equal(true, bool0_47)
	...
	dataGetBoard0_48 := &content.BackendGetBoard{}
	testCore(t0, bodyString, dataGetBoard0_48, t, isDebug)

	assert.Equal(types.StatusTerminal, dataGetBoard0_48.Status)

	dataGetBoard1_48 := &content.BackendGetBoard{}
	testCore(t1, bodyString, dataGetBoard1_48, t, isDebug)

	assert.Equal(types.StatusTerminal, dataGetBoard1_48.Status)
	...
	assert.Equal(0, len(dataGetArticleList0_49.Result))

	dataGetArticleList1_49 := &struct {
		Result []*content.BackendGetArticle `json:"result"`
	}{}
	testListCore(t1, bodyString, dataGetArticleList1_49, t, isDebug)
	assert.Equal(0, len(dataGetArticleList1_49.Result))
