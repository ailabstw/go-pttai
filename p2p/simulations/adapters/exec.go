...
func execP2PNode() {
	...
	services := make(map[string]pkgservice.PttService, len(serviceNames))
	for _, name := range serviceNames {
		serviceFunc, exists := serviceFuncs[name]
		if !exists {
			log.Crit("unknown node service", "name", name)
		}
		constructor := func(nodeCtx *pkgservice.ServiceContext) (pkgservice.PttService, error) {
			ctx := &ServiceContext{
	...
	if err := stack.Register(func(ctx *pkgservice.ServiceContext) (pkgservice.PttService, error) {
...
type snapshotService struct {
	services map[string]pkgservice.PttService
}
