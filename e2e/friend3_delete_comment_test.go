...
func TestFriend3DeleteComment(t *testing.T) {
	NNodes = 3
	isDebug := true
	...
	var board1_0_16 *content.BackendGetBoard
	var board1_2_16 *content.BackendGetBoard

	if reflect.DeepEqual(board1_16_1.ID, me0_3.BoardID) {
		board1_0_16 = board1_16_1
		board1_2_16 = board1_16_2
	} else {
		board1_0_16 = board1_16_2
		board1_2_16 = board1_16_1
	...
	// 17.1. count peers1
	marshaled, _ = board1_0_16.ID.MarshalText()
	bodyString = fmt.Sprintf(`{"id": "testID", "method": "content_countPeers", "params": ["%v"]}`, string(marshaled))
	...
	// 17.2. count peers2
	marshaled, _ = board1_2_16.ID.MarshalText()
	bodyString = fmt.Sprintf(`{"id": "testID", "method": "content_countPeers", "params": ["%v"]}`, string(marshaled))

	count2_17_2, _ := testIntCore(t2, bodyString, t, isDebug)
	...
	count0_17_3, _ := testIntCore(t0, bodyString, t, isDebug)
	assert.Equal(1, count0_17_3)

	count1_17_3, _ := testIntCore(t1, bodyString, t, isDebug)
	assert.Equal(2, count1_17_3)

	count2_17_3, _ := testIntCore(t0, bodyString, t, isDebug)
	assert.Equal(1, count2_17_3)
	...
	marshaledID, _ = board1_16_3.ID.MarshalText()
	marshaledID2, _ = article0_36.ID.MarshalText()
	marshaledID3, _ = articleBlock1_46_5.RefID.MarshalText()
	...
	assert.Equal(7, len(dataGetArticleBlockList2_49.Result))

	articleBlock2_49_5 := dataGetArticleBlockList2_49.Result[5]
	assert.Equal(types.StatusDeleted, articleBlock2_49_5.Status)
	assert.Equal(content.ContentTypeComment, articleBlock2_49_5.ContentType)
	assert.Equal(content.CommentTypeBoo, articleBlock2_49_5.CommentType)
	assert.Equal(content.DefaultDeletedComment, articleBlock2_49_5.Buf)
	...
	testListCore(t1, bodyString, dataGetArticleList2_50, t, isDebug)
	assert.Equal(1, len(dataGetArticleList2_50.Result))
	article2_50 := dataGetArticleList2_50.Result[0]

	assert.Equal(3, article2_50.NPush)
	assert.Equal(1, article2_50.NBoo)
	assert.Equal(types.StatusAlive, article2_50.Status)
}
