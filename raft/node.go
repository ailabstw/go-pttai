...
type Node interface {
	...
	// ForceProposeConfChange proposes config change without consensus.
	// At most one ConfChange can be in the process of going through consensus.
	// Application needs to call ApplyConfChange when applying EntryConfChange type entry.
...
func RestartNode(c *Config) Node {
	log.Debug("RestartNode: to newRaft")

	r := newRaft(c)

	log.Debug("RestartNode: after newRaft")

	n := newNode()
	n.logger = c.Logger

	log.Debug("RestartNode: after newNode")
	go n.run(r)

	log.Debug("RestartNode: after run")
...
func (n *node) Tick() {
	...
	case <-n.done:
	default:
		log.Warn("A tick missed to fire. Node blocks too long!")
	}
}
