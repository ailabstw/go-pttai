...
func TestFriendDeleteFriend4(t *testing.T) {
	NNodes = 2
	isDebug := true
	...
	dataJoinFriend0_18 := &pkgservice.BackendJoinRequest{}
	testCore(t0, bodyString, dataJoinFriend0_18, t, isDebug)

	assert.Equal(me1_3.ID, dataJoinFriend0_18.CreatorID)
	assert.Equal(me1_1.NodeID, dataJoinFriend0_18.NodeID)
	...
	dataGetFriendList0_19 := &struct {
		Result []*friend.BackendGetFriend `json:"result"`
	}{}
	testListCore(t0, bodyString, dataGetFriendList0_19, t, isDebug)
	assert.Equal(1, len(dataGetFriendList0_19.Result))
	friend0_19 := dataGetFriendList0_19.Result[0]
	assert.Equal(types.StatusAlive, friend0_19.Status)
	assert.Equal(me1_1.ID, friend0_19.FriendID)

	dataGetFriendList1_19 := &struct {
		Result []*friend.BackendGetFriend `json:"result"`
	}{}
	testListCore(t1, bodyString, dataGetFriendList1_19, t, isDebug)
	assert.Equal(1, len(dataGetFriendList1_19.Result))
	friend1_19 := dataGetFriendList1_19.Result[0]
	assert.Equal(types.StatusAlive, friend1_19.Status)
	assert.Equal(me0_1.ID, friend1_19.FriendID)
	assert.Equal(friend0_19.ID, friend1_19.ID)

	// 20. get-raw-friend
	marshaled, _ = friend0_19.ID.MarshalText()
	bodyString = fmt.Sprintf(`{"id": "testID", "method": "friend_getRawFriend", "params": ["%v"]}`, string(marshaled))

	friend0_20 := &friend.Friend{}
	testCore(t0, bodyString, friend0_20, t, isDebug)
	assert.Equal(friend0_19.ID, friend0_20.ID)
	assert.Equal(me1_1.ID, friend0_20.FriendID)
	assert.Equal(me0_1.ID, friend0_20.OwnerIDs[0])

	friend1_20 := &friend.Friend{}
	testCore(t1, bodyString, friend1_20, t, isDebug)
	assert.Equal(friend1_19.ID, friend1_20.ID)
	assert.Equal(friend0_20.Friend0ID, friend1_20.Friend0ID)
	assert.Equal(friend0_20.Friend1ID, friend1_20.Friend1ID)
	assert.Equal(me0_1.ID, friend1_20.FriendID)
	assert.Equal(me1_1.ID, friend1_20.OwnerIDs[0])
	...
	testCore(t0, bodyString, &bool0_21, t, isDebug)
	assert.Equal(true, bool0_21)
	...
	dataGetFriendList0_22 := &struct {
		Result []*friend.BackendGetFriend `json:"result"`
	}{}
	testListCore(t0, bodyString, dataGetFriendList0_22, t, isDebug)
	assert.Equal(1, len(dataGetFriendList0_22.Result))
	friend0_22_0 := dataGetFriendList0_22.Result[0]
	assert.Equal(types.StatusTerminal, friend0_22_0.Status)

	dataGetFriendList1_22 := &struct {
		Result []*friend.BackendGetFriend `json:"result"`
	}{}
	testListCore(t1, bodyString, dataGetFriendList1_22, t, isDebug)
	assert.Equal(1, len(dataGetFriendList1_22.Result))
	friend1_22_0 := dataGetFriendList1_22.Result[0]
	assert.Equal(types.StatusTerminal, friend1_22_0.Status)
}
