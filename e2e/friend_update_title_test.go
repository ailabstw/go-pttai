...
func TestFriendUpdateTitle(t *testing.T) {
	NNodes = 2
	isDebug := true
	...
	title0_10_1 := []byte("標題1")
	marshaledStr = base64.StdEncoding.EncodeToString(title0_10_1)
	bodyString = fmt.Sprintf(`{"id": "testID", "method": "content_setTitle", "params": ["%v", "%v"]}`, string(marshaledID), marshaledStr)

	dataSetTitle0_10_1 := &content.BackendGetBoard{}
	testCore(t0, bodyString, dataSetTitle0_10_1, t, isDebug)

	// 10.2. get board list
	bodyString = fmt.Sprintf(`{"id": "testID", "method": "content_getBoardList", "params": ["", 0, 2]}`)

	dataBoardList0_10_2 := &struct {
		Result []*content.BackendGetBoard `json:"result"`
	}{}

	testListCore(t0, bodyString, dataBoardList0_10_2, t, isDebug)
	assert.Equal(1, len(dataBoardList0_10_2.Result))
	board0_10_2_0 := dataBoardList0_10_2.Result[0]
	assert.Equal(me0_3.BoardID, board0_10_2_0.ID)
	assert.Equal(types.StatusAlive, board0_10_2_0.Status)

	assert.Equal(title0_10_1, board0_10_2_0.Title)

	// 11.
	marshaledID, _ = board0_10_0.ID.MarshalText()
	bodyString = fmt.Sprintf(`{"id": "testID", "method": "content_getArticleList", "params": ["%v", "", 0, 2]}`, string(marshaledID))

	dataGetArticleList0_11 := &struct {
		Result []*content.BackendGetArticle `json:"result"`
	}{}
	testCore(t0, bodyString, dataGetArticleList0_11, t, isDebug)

	assert.Equal(0, len(dataGetArticleList0_11.Result))
	...
	dataGetBoard0_49 := &content.BackendGetBoard{}
	testCore(t0, bodyString, dataGetBoard0_49, t, isDebug)
	assert.Equal(title0_10_1, dataGetBoard0_49.Title)

	dataGetBoard1_49 := &content.BackendGetBoard{}
	testCore(t1, bodyString, dataGetBoard1_49, t, isDebug)
	assert.Equal(title0_10_1, dataGetBoard1_49.Title)
