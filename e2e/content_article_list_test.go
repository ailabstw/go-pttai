...
func TestContentArticleList(t *testing.T) {
	NNodes = 1
	isDebug := true
	...
	// 39. create-article
	article, _ = json.Marshal([]string{
		base64.StdEncoding.EncodeToString([]byte("測試23")),
		base64.StdEncoding.EncodeToString([]byte("測試24")),
		base64.StdEncoding.EncodeToString([]byte("測試25")),
		base64.StdEncoding.EncodeToString([]byte("測試26")),
		base64.StdEncoding.EncodeToString([]byte("測試27")),
		base64.StdEncoding.EncodeToString([]byte("測試28")),
		base64.StdEncoding.EncodeToString([]byte("測試29")),
		base64.StdEncoding.EncodeToString([]byte("測試30")),
	})
	...
	// 41. content-get-article-list
	marshaledID, _ = board0_10_0.ID.MarshalText()

	bodyString = fmt.Sprintf(`{"id": "testID", "method": "content_getArticleList", "params": ["%v", "", 0, 1]}`, string(marshaledID))
	dataGetArticleList0_41 := &struct {
		Result []*content.BackendGetArticle `json:"result"`
	}{}
	testListCore(t0, bodyString, dataGetArticleList0_41, t, isDebug)
	assert.Equal(2, len(dataGetArticleList0_41.Result))

	article0_41_0 := dataGetArticleList0_41.Result[0]
	article0_41_1 := dataGetArticleList0_41.Result[1]

	assert.Equal(article0_40_1, article0_41_0)
	assert.Equal(article0_40_0, article0_41_1)
}
