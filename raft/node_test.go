...
func TestDisableProposalForwarding(t *testing.T) {
	...
	log.Debug("TestDisablePropsoalForwarding: start")
	defer log.Debug("TestDisablePropsoalForwarding: done")
	...
	log.Debug("TestDisablePropsoalForwarding", "r2", r2)
...
func TestNodeProposeAddDuplicateNode(t *testing.T) {
	...
	log.Debug("TestNodeProposeAddDuplicateNode: start")
	defer log.Debug("TestNodeProposeAddDuplicateNode: done")
	...
	log.Debug("TestNodeProposeAddDuplicateNode: to propose dup-conf-change")

	// try add the same node again
	n.ProposeConfChange(context.TODO(), cc1)
	<-applyConfChan

	log.Debug("TestNodeProposeAddDuplicateNode: to propose cc2")
	...
	log.Debug("TestNodeProposeAddDuplicateNode: after propose cc2")
...
func TestReadyContainUpdates(t *testing.T) {
	...
	log.Debug("TestReadyContainUpdates: start")
	defer log.Debug("TestReadyContainUpdates: done")
...
func TestNodeStart(t *testing.T) {
	...
	log.Debug("TestNodeStart: start")
	defer log.Debug("TestNodeStart: done")
...
func TestNodeProposeAddLearnerNode(t *testing.T) {
	...
	go func() {
		...
		for {
			...
			case rd := <-n.Ready():
				s.Append(rd.Entries)
				t.Logf("TestNodeProposeAddLearnerNode: rd.Entries: %v", rd.Entries)
				for _, ent := range rd.Entries {
					...
					if len(state.Learners) == 0 ||
						...
						cc.NodeID != 2 {
						t.Errorf("apply conf change should return new added learner: %v state.learners: %v cc.NodeID: %v cc: %v", state.String(), state.Learners, cc.NodeID, cc)
					}

					if len(state.Nodes) != 1 {
						t.Errorf("add learner should not change the nodes: %v state.Nodes: %v", state.String(), state.Nodes)
					}
